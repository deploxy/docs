...
```mermaid
graph TD
    A[Your MCP Server Code] -- Build --> B(Deploxy CLI);
    B -- Upload --> Deploxy-Cloud{Deploxy Cloud};
    subgraph Deploxy-Cloud
        D[Deploy] --> E["Serverless Function 
        (Your MCP Server Code)"];
        D --> F[Generate Proxy Code];
    end
    F -- Publish @your-org/mcp --> G[NPM Registry];
    H[End User] -- "npx @your-org/mcp" --> G;
    G -- "npm install" --> I[Proxy Client];
    I -- MCP Call --> E;
