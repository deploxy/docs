...
An MCP Operation includes only **Call Operations**:

-   **Call Operation:** A request to execute a tool, resource, or prompt.

<Note>
**List Operations are Free:** Requests to list available resources, prompts, or tools are not counted as MCP Operations and do not consume your usage quota.
</Note>
...
**`Consumed MCP Operations = Number of Call Requests * (Memory Size in MB / 256 MB)`**

For example:
-   A **call operation** to a function with **256MB** of memory consumes **1** MCP Operation.
-   A **call operation** to a function with **512MB** of memory consumes **2** MCP Operations.
-   **List operations** (listing tools, resources, or prompts) consume **0** MCP Operations regardless of memory size.

This model ensures you only pay for the computational resources you actually use, while keeping discovery operations free.
...
-   **Free 5,000 MCP Operations per Month:** A generous allowance for development and small-scale applications. Remember, only call operations count - list operations are always free.
...
-   **Free 10,000 MCP Operations per Month:** A larger free allowance for your applications. Only call operations are counted - list operations remain free.
