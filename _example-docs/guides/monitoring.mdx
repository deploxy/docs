...
summary: 'Learn how to monitor your deployments and access logs.'
sequence: 6
---
...
### Using the Deploxy CLI

For programmatic access or to view logs in your terminal, use the `deploxy logs` command.

<CodeBlock title="Terminal" language="bash">
```bash
# Follow logs in real-time for a specific deployment
npx @deploxy/cli logs <deployment-id> --follow

# Fetch the last 100 lines of error logs
npx @deploxy/cli logs <deployment-id> --level error --tail 100
```
</CodeBlock>
